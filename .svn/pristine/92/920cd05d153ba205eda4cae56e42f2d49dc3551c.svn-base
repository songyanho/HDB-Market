...
class AuthRoute{
    ...
    public function isAllowedByUser($user){
        $role = HdbCore::ROLE_GUEST;
        if($user === false)
            return false;
        switch(get_class($user)){
            case 'CallOperator':
                $role = HdbCore::ROLE_CALL_OPERATOR;
                break;
            case 'Agency':
                $role = HdbCore::ROLE_AGENCY;
                break;
            case 'KeyDecisionMaker':
                $role = HdbCore::ROLE_KEY_DECISION_MAKER;
                break;
            case 'MINISTER':
                $role = HdbCore::ROLE_MINISTER;
                break;
            default:
                $role = HdbCore::ROLE_GUEST;
                break;
