...
abstract class User implements ActiveRecordInterface
{
    ...
    /**
     * @var        ChildRealtor one-to-one related ChildRealtor object
     */
    protected $singleRealtor;

    /**
     * @var        ChildNormalUser one-to-one related ChildNormalUser object
     */
    protected $singleNormalUser;
    ...
    public function setDescendantClass($v)
    {
        ...
        if ($this->descendant_class !== $v) {
            ...
            $this->modifiedColumns[UserTableMap::COL_DESCENDANT_CLASS] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : UserTableMap::translateFieldName('DescendantClass', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        if ($deep) {  // also de-associate any related objects?
            ...
            $this->singleRealtor = null;

            $this->singleNormalUser = null;
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->singleRealtor !== null) {
                if (!$this->singleRealtor->isDeleted() && ($this->singleRealtor->isNew() || $this->singleRealtor->isModified())) {
                    $affectedRows += $this->singleRealtor->save($con);
                }
            }

            if ($this->singleNormalUser !== null) {
                if (!$this->singleNormalUser->isDeleted() && ($this->singleNormalUser->isNew() || $this->singleNormalUser->isModified())) {
                    $affectedRows += $this->singleNormalUser->save($con);
                }
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_DESCENDANT_CLASS)) {
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            ...
            $keys[6] => $this->getDescendantClass(),
        );
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->singleRealtor) {
                ...
                $result[$key] = $this->singleRealtor->toArray($keyType, $includeLazyLoadColumns, $alreadyDumpedObjects, true);
            }
            if (null !== $this->singleNormalUser) {
                ...
                $result[$key] = $this->singleNormalUser->toArray($keyType, $includeLazyLoadColumns, $alreadyDumpedObjects, true);
            }
        }
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[6], $arr)) {
            $this->setDescendantClass($arr[$keys[6]]);
        }
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_DESCENDANT_CLASS)) {
            $criteria->add(UserTableMap::COL_DESCENDANT_CLASS, $this->descendant_class);
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            $relObj = $this->getRealtor();
            if ($relObj) {
                $copyObj->setRealtor($relObj->copy($deepCopy));
            }

            $relObj = $this->getNormalUser();
            if ($relObj) {
                $copyObj->setNormalUser($relObj->copy($deepCopy));
            }
     ...
     * @return ObjectCollection|ChildWatchlist[] List of ChildWatchlist objects
     */
    public function getWatchlistsJoinListing(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildWatchlistQuery::create(null, $criteria);
        $query->joinWith('Listing', $joinBehavior);

        return $this->getWatchlists($query, $con);
    }

    /**
     * Gets a single ChildRealtor object, which is related to this object by a one-to-one relationship.
     *
     * @param  ConnectionInterface $con optional connection object
     * @return ChildRealtor
     * @throws PropelException
    ...
    public function getRealtor(ConnectionInterface $con = null)
    {
        ...
        if ($this->singleRealtor === null && !$this->isNew()) {
            $this->singleRealtor = ChildRealtorQuery::create()->findPk($this->getPrimaryKey(), $con);
        }

        return $this->singleRealtor;
    }

    /**
     * Sets a single ChildRealtor object as related to this object by a one-to-one relationship.
     *
     * @param  ChildRealtor $v ChildRealtor
     * @return $this|\User The current object (for fluent API support)
     * @throws PropelException
     */
    public function setRealtor(ChildRealtor $v = null)
    {
        $this->singleRealtor = $v;

        // Make sure that that the passed-in ChildRealtor isn't already associated with this object
    ...
    /**
     * Gets a single ChildNormalUser object, which is related to this object by a one-to-one relationship.
     *
     * @param  ConnectionInterface $con optional connection object
     * @return ChildNormalUser
     * @throws PropelException
     */
    public function getNormalUser(ConnectionInterface $con = null)
    {

        if ($this->singleNormalUser === null && !$this->isNew()) {
            $this->singleNormalUser = ChildNormalUserQuery::create()->findPk($this->getPrimaryKey(), $con);
        }

        return $this->singleNormalUser;
    }

    /**
     * Sets a single ChildNormalUser object as related to this object by a one-to-one relationship.
     *
     * @param  ChildNormalUser $v ChildNormalUser
     * @return $this|\User The current object (for fluent API support)
     * @throws PropelException
     */
    public function setNormalUser(ChildNormalUser $v = null)
    {
        $this->singleNormalUser = $v;

        // Make sure that that the passed-in ChildNormalUser isn't already associated with this object
        if ($v !== null && $v->getUser(null, false) === null) {
            $v->setUser($this);
        }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep) {
            ...
            if ($this->singleRealtor) {
                $this->singleRealtor->clearAllReferences($deep);
            }
            if ($this->singleNormalUser) {
                $this->singleNormalUser->clearAllReferences($deep);
            }
        } // if ($deep)
